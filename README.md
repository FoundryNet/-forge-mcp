...
> Industrial machine agent control plane. Talk to any CNC, robot, or industrial PLC in plain English.
...
11 Model Context Protocol tools wrapping the [Forge](https://foundrynet.io) v1 API. Cross-manufacturer telemetry normalization across **16 OEM families** with **18,000+ field mappings**: Fanuc, Siemens, Haas, DMG Mori, Mazak, Okuma, Doosan, Makino, ABB, KUKA, Universal Robots, Yaskawa, Stäubli, Trumpf, Bystronic, Bosch Rexroth. Every state-changing action is cryptographically anchored on Solana mainnet via the [MINT Protocol](https://foundrynet.io) relay, so any third party can verify *the action happened with these inputs against this machine* via a Solscan link — no trust in the server required.
...
| **Free tier** | 100 tool calls/month, 4 read-only tools |
| **Pro** | $49/month — all 11 tools, 10,000 calls/month |
